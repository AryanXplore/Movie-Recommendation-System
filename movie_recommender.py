...
def recommend_by_genre():
    ...
    # Get genres from the target movie
    target_genres = [genre.strip().lower() for genre in target_movie['genre'].split(',')]
    
    # Find movies with similar genres (excluding the original movie)
    similar_movies = []
    for movie in movies:
        if movie['id'] == movie_id:  # Skip the original movie
            continue
    ...
    # Sort by vote_count and return top 10
    similar_movies_sorted = sorted(similar_movies, key=lambda x: x['vote_count'], reverse=True)[:10]
    return jsonify(similar_movies_sorted)
